...
void Pixel_2_LinCol(int *lin, int *col, int x, int y, int Inicio_x, int Inicio_y, int Tam_x, int Tam_y){
    ...
    int dir = ((Pos_X_Carta % Tam_x) > (Tam_x/2)) ? 1 : 0;
    ...
    g_print("\t lin2: %d, col2: %d, x_Pos_P_tam: %d, x_Pos_P_tam: %d, Tam_x: %d, Tam_y: %d\n", linha, coluna, Pos_X_Carta % Tam_x, Pos_Y_Carta % Tam_y, Tam_x, Tam_y);
...
int valida_jogada (JOGADORES_PTR Jogador, LISTA_CARTAS_PTR *Backup_Mao, LISTA_MESA_PTR *mesa_backup, LISTA_MESA_PTR *mesa_nova){
    LISTA_CARTAS_PTR mao_atual = Jogador->cartas;
    while(mao_atual != NULL){
        if(Busca_Carta(Backup_Mao, mao_atual->naipe, mao_atual->numero, mao_atual->interacao) == NULL)
            break;
        
        mao_atual = mao_atual->prox;
    }
    if(mao_atual == NULL){
